...
function addToCart(buttonElement) {
    ...
    if (category === 'chicken' || (category === 'meat' && name === 'نفر لحم مندي')) {
        ...
        // 🔴🔴🔴 التعديل الحاسم: تمرير البيانات إلى النافذة المنبثقة 🔴🔴🔴
...
// --- وظائف نافذة الخيارات (منطق التسعير المحدث) ---
...
function updateOptionTotal() {
    ...
    // قراءة اسم الصنف لتحديد قاعدة التسعير
    ...
    if(sizeInput) {
        ...
        // نستخدم basePrice + sizeModifier لتطبيق تعديل الحجم على السعر الأصلي
        finalPrice = basePrice + parseFloat(sizeInput.dataset.priceModifier || 0);
        
        // تحديث وصف السعر في خيارات الرز ليتناسب مع الصنف والحجم
        document.querySelectorAll('.rice-beshawer-price, .rice-mandi-price, .rice-mathloutha-price').forEach(el => {
            if (itemName === 'نفر لحم مندي') {
                 el.textContent = 'إضافة 5'; // للنفر 5 ريال
            } else {
                 el.textContent = sizeText === 'نص' ? 'إضافة 1' : 'إضافة 4'; // للدجاج 1 أو 4 ريال
            }
        });
    }

    // 2. تحديد نوع الرز وإضافة سعر التعديل (المنطق الجديد)
    if(riceInput) {
        if (itemName === 'نفر لحم مندي') {
