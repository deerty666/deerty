...
/*************************************************
 * تعديل دالة الطباعة لتلبية متطلباتك
 *************************************************/
function printReceipt() {
    // 1. بناء جدول الأصناف مع السعر الفردي وإجمالي الصنف
    let itemsRows = '';
    cart.forEach(item => {
        const itemUnitPrice = item.price.toFixed(2); // السعر الفردي
        const itemTotal = (item.price * item.qty).toFixed(2); // إجمالي الصنف
        itemsRows += `
            <tr style="border-bottom: 1px dashed #ddd; font-size: 14px;">
                <td style="padding: 10px 0; font-weight: bold;">${item.name}</td>
                <td style="text-align: center;">${item.qty}</td>
                <td style="text-align: left;">${itemUnitPrice} ر.س</td>
    ...
    win.document.write(`
        ...
        <head>
            ...
            <style>
                /* ضبط حجم الصفحة لطابعة حرارية 80 ملم */
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="text-align: left;">الفردي</th>
