...
let deliveryFee = 15;
let openCategory = null;   // لتتبع القسم المفتوح حالياً

function saveMenu() {
    localStorage.setItem('menu', JSON.stringify(menu));
}

// عرض القائمة مع خاصية Accordion
function renderMenu() {
    ...
    menu.categories.forEach((cat, index) => {
        ...
        // حاوية الأصناف (تظهر فقط إذا كان مفتوحاً)
...
// فتح وإغلاق القسم (Accordion)
function toggleCategory(cat) {
    if (openCategory === cat) {
        openCategory = null;   // إغلاق نفس القسم
    } else {
        openCategory = cat;    // فتح القسم الجديد وإغلاق السابق
    }
    renderMenu();
}
