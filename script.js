...
function sendToWhatsApp() {
    ...
    // 1️⃣ تجميع الأصناف بطريقة صحيحة
    let itemsText = '';
    cart.forEach(item => {
        const itemTotal = item.price * item.qty;
    ...
    // 2️⃣ صياغة نص الرسالة باستخدام علامة الـ Backtick (`)
...
// ====================== نافذة الإدارة (مع تعديل وحذف الصنف) ======================
...
function renderAdminCategories() {
    ...
    menu.categories.forEach(cat => {
        ...
        // عرض الأصناف داخل القسم مع زر تعديل وحذف
        const catItems = menu.items.filter(i => i.category === cat);
        catItems.forEach(item => {
...
function deleteCategory(catName) {
    if (!confirm(`حذف القسم "${catName}" وجميع أصنافه؟`)) return;
