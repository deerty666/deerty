...
function renderMenu() {
    ...
    menu.categories.forEach(cat => {
        ...
        catDiv.style.background = '#3498db';
...
function renderCart() {
    ...
    cart.forEach((item, index) => {
        ...
        div.innerHTML = `
            ...
            <input type="text" placeholder="ملاحظة" value="\( {item.note}" onchange="cart[ \){index}].note=this.value" style="width:100%;margin:8px 0;padding:8px;">
...
function addItemFromAdmin() {
    ...
    if (!name || !price || !cat) return alert("املأ الحقول");
