...
function sendToWhatsApp() { /* نفس الدالة السابقة */ 
    ...
    let itemsText = cart.map(i => `- ${i.name} × ${i.qty} = ${i.price*i.qty} ريال ${i.note ? '('+i.note+')' : ''}`).join('\n');
...
function clearCart() {
    if (confirm("مسح الفاتورة؟")) { cart = []; renderCart(); }
}
...
function renderAdminCategories() {
    ...
    menu.categories.forEach((cat, index) => {
        ...
        div.innerHTML = `
            <span>${cat}</span>
            <button onclick="deleteCategory('${cat}')" style="background:#e74c3c;color:white;padding:5px 10px;border:none;border-radius:5px;">حذف</button>
        `;
        container.appendChild(div);
    });
}
...
function addCategory() {
    ...
    renderMenu();
    renderAdminCategories();
    renderAdminSelect();          // ← هذا التحديث المهم
